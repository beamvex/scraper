...
fn main() -> Result<()> {
    ...
    for (_, page) in &pages {
        ...
        if needs_rebuild(page, &html) {
            ...
            println!("skipping {} (up to date)", page.display());
        }
    }
    ...
    if processed > 0 {
        ...
        println!("no new pages to deploy");
    }
    Ok(())
}

fn needs_rebuild(page: &Path, html: &Path) -> bool {
    let page_mtime = fs::metadata(page)
        .and_then(|m| m.modified())
        .unwrap_or(SystemTime::UNIX_EPOCH);
    let html_mtime = fs::metadata(html)
        .and_then(|m| m.modified())
        .unwrap_or(SystemTime::UNIX_EPOCH);
    page_mtime > html_mtime
}
