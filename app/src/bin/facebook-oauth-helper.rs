...
<html>
  ...
  <body>
    ...
    <form method="post" action="/start">
      <div class="row">
        <label>App ID</label>
        <input name="app_id" type="text" placeholder="facebook app id" required />
      </div>

      <div class="row">
        <label>App Secret</label>
        <input name="app_secret" type="password" placeholder="facebook app secret" required />
      </div>

      <div class="row">
        <label>Redirect URI</label>
        <input name="redirect_uri" type="text" value="http://127.0.0.1:8086/callback" required />
        <div class="hint">Must exactly match the redirect URI configured in your Facebook app.</div>
      </div>

      <div class="row">
        <label>Scopes</label>
        <input name="scope" type="text" value="pages_manage_posts,pages_read_engagement,pages_show_list" required />
        <div class="hint">You may need <code>pages_manage_posts</code> + <code>pages_show_list</code>. App must be in Live mode / have permissions approved depending on use.</div>
      </div>

      <div class="row">
        <label>Page ID (optional)</label>
        <input name="page_id" type="text" placeholder="if blank, we use the first page from /me/accounts" />
      </div>
...
async fn start_inner(state: AppState, form: StartForm) -> Result<axum::response::Response> {
    if form.app_id.trim().is_empty() {
        bail!("app_id is required");
    }
    if form.app_secret.trim().is_empty() {
...
async fn callback_inner(state: AppState, params: CallbackParams) -> Result<axum::response::Response> {
    ...
    {
        let mut inner = state.inner.lock().unwrap();
        inner.last_json = Some(final_json);
    }
...
async fn exchange_code_for_short_lived_token(cfg: &Config, code: &str) -> Result<String> {
    ...
    if !status.is_success() {
        bail!("facebook token exchange failed: HTTP {}: {}", status, text);
    }
...
async fn exchange_for_long_lived_token(cfg: &Config, short: &str) -> Result<String> {
    ...
    let resp = CLIENT
        ...
        .send()
        .await
        .context("facebook long-lived token request failed")?;

    let status = resp.status();
    let text = resp.text().await.context("failed to read long-lived response")?;
    if !status.is_success() {
        bail!("facebook long-lived token exchange failed: HTTP {}: {}", status, text);
    }
    ...
    let token = v
        .get("access_token")
        .and_then(|x| x.as_str())
        .context("missing access_token in long-lived response")?;
    Ok(token.to_string())
}
